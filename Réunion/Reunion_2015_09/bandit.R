...
while(t <= Round){
  ...
  resTAUX <- getValPersNEWYANN(duration=t*TStep*365, nbVilles=as.numeric(drawIt),
...
vecReward2D <-as.data.frame(vecReward2D); names(vecReward2D)<-namesRow
vecweights2D <- as.data.frame(vecweights2D); names(vecweights2D)<-namesRow
