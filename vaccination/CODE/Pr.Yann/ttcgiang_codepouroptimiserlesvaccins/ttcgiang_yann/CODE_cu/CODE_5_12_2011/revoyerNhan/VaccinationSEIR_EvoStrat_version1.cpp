...
{
	cout<<"Bonjour Bienvenue Vaccination SEIR utilisant l'Algorithme (1+1)-ES"<<endl;
	SAUVER[0]='N';
	int i,j;
        cout << "----\n";
	// mon objectif: reussir a diminuer le nombre d'infectes en utilisant 2000 vaccins au plus.
	nombreMaxVaccine = 2000;
	...
	cout<<"Valeurs initiales de vecteur politique!"<<endl;
	for(int m =0; m<politique.size(); m++)
	cout<<politique[m]<<endl;

	for (i=0; i < 5; i++) {
	fitness_sequence.clear();
        cout<<"Valeurs de la vecteur politique de la fois "<<i<<endl;
        cout<<"double = "<<&eval_policy<<endl;
        //cac politique lien quan den nhau politique 1 anh huong den politique2,....
        for(int m =0; m<politique.size(); m++)	cout<<"Avant     "<<politique[m]<<endl;
        cout<<"Avant nombreTotalInfectes = "<<nombreTotalInfectes<<endl;

	politique = evo_strat(politique,&eval_policy,5,1.0,true,&fitness_sequence);

        cout<<"Apres nombreTotalInfectes = "<<nombreTotalInfectes<<endl;
        for(int m =0; m<politique.size(); m++)	cout<<"Apres    "<<politique[m]<<endl;
...
int vaccinerToutesVilles_V1(double tauxVaccToutesVilles,int nbVilles, unsigned long **y,double_vector *politique)
{
    unsigned long nbPersonneVaccine=0;
        int i=0;
cout<<"Trong ham vacciner "<<endl;
//while((*politique)[i] !=NULL){
  //  cout<<"politique"<<i<<" = "<<(*politique)[i]<<endl;
//}
