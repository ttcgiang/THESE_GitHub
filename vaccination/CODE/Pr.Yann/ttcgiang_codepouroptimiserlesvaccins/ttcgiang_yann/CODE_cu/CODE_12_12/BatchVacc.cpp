...
int main(int argc, char ** argv)
{
    ...
    if((strcmp(SAUVER,"Y"))==0){
        ...
        for(int i=1; i<= nbVill;i++){
            sprintf(nameFile,"Output/SimVil%dEps%.3fTop%d.csv",i,epsilon,topology);
    ...
    sprintf(nameFileTotal,"Output/tolSEIRNbVil%dEps%.3fTop%dGam%.2fSig%.2f.csv",nbVill,epsilon,topology,gamma,sigma);
...
void AfficheArgument(int nbVill,unsigned long rinitN, double epsilon, double tmax,int argc, char **argv){
    ...
    char nameFile[100];
    char line[200];
    sprintf(nameFile,"%s%d%s%ld%s%.3f%s%.1f%s","Output/SimuNbVille",nbVill,"Ninit",rinitN,"Epsilon",epsilon,"Tmax",tmax,"_PARAMS.csv");
    pFile = fopen (nameFile,"w");
