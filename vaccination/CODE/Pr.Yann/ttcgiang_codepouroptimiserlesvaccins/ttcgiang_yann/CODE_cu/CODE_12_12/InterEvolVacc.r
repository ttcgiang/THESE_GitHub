...
fEvolVaccParams <- paste("./EvolVaccParms.txt",Sys.Date(),".txt",seq="")


InterEvolVacc <- function(
...
repeat
{
print("\n Voullez-vous changer la valer defaut de parametres (y(Y)/n(N)?")
...
EvolVaccParms=paste(text1, text2, text3,text4,text5,text6,text7,text8,text9,text10, text11, text12, text13, text16,text17,text18,text19, text20, text21, text22,text23)


#Creer un chemin pour fonctionner le programme 
StartEvolVaccProcess<-paste(Simulator,EvolVaccParms)
system(StartEvolVaccProcess)

#Modifie par ttcgiang 9/11
#Sauvegarder les valeurs des arguments dans fichier.txt
print("Est-ce que vous voulez sauvegarder les valeurs des arguments que vous vien d'utiliser pour simulateur, au fichier general EvolVaccParms.txt (y(Y)/n(N)?")
lettres <-scan(what="complex")
nlettres <- length(lettres)
lettre <- lettres[nlettres]
if((lettre =='y')||(lettre=='Y'))
{
write(EvolVaccParms, file = fEvolVaccParams,append=TRUE, sep = "\n")
}
else 
{
...
#pdf general
print("Est-ce que vous voulez executer toutes les valeurs des arguments des simulateurs que vous avez sauvegardes dans le fichier general EvolVaccParms.txt (y(Y)/n(N)?")
lettres <-scan(what="complex")
nlettres <- length(lettres)
lettre <- lettres[nlettres]
if((lettre =='y')||(lettre=='Y'))
{
arrEvolVaccParms <- readLines(fEvolVaccParams, n=-1)
nEvolVaccParms <- length(arrEvolVaccParms)
for( iEvolvacc in 1:nEvolVaccParms)
{
StartEvolVaccProcess<-paste(Simulator,arrEvolVaccParms[iEvolvacc])
system(StartEvolVaccProcess)
}
#The end if general
}
...
{
print("Finir le programme InetrEvolVacc.r!")
}
# The end
}
