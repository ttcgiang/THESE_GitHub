...
double_vector evo_strat(double_vector*v0,
                        vector<double> valParSIM, char *SAUVER, vector<nodeSEIR>* valeursSEIR0, double_vector *valParVAC,
                        int ntrials=1000, double sigma=1.0, bool adaptive=false, double_vector *fitness_sequence=NULL)
{
	void add_noise_to_vector(double_vector &,double);
        if(v0){
            double_vector v=*v0;
            int	   t,s=v.size();
            double eval_v = eval_policy(valParSIM,SAUVER,valeursSEIR0,valParVAC,&v);
            ...
            for (t=1; t <= ntrials; t++) {
                    ...
                    eval_v2 =  eval_policy(valParSIM,SAUVER,valeursSEIR0,valParVAC,&v);;
                    ...
                    if (fitness_sequence)	fitness_sequence->push_back(eval_v);
                    //if (fitness_sequence)	fitness_sequence->push_back(eval_v2);
        ...
        else
        {
           ...
           // double_vector v = *v0;
            for (t=0; t <= ntrials; t++) {
                    double eval_v;
                    eval_v =  eval_policy(valParSIM,SAUVER,valeursSEIR0,valParVAC,NULL);;
                    if (fitness_sequence)	fitness_sequence->push_back(eval_v);
            }
