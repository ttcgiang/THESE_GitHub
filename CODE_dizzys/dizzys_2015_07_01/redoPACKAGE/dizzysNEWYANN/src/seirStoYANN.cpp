...
*/
  ...
  */
   ...
   */
    ...
    void initializeStatSEIRYANN(int nbVilles, double* statMatrix, unsigned long S0, unsigned long I0, unsigned long E0, unsigned long R0,
                            unsigned long **&y){
        for(int i=0; i<nbVilles; i++){
   ...
   */
           ...
           if(event != -1){
               ...
               // Check the local extinction and the stop of the local extinction
                for(int i= 0; i< nbVilles; i++){
                    if((y[i][iI]==0)&& (vecFlagExtLocal[i]==false)){
                        tableExtLocal[i].push_back(t);
                        vecFlagExtLocal[i]=true;
                    }
                    if((y[i][iI]!=0) && (vecFlagExtLocal[i]==true)){
