...
SEXP getValeurPOPSYANN(SEXP sigma, SEXP gamma,SEXP mu,SEXP seed,
                    SEXP S0, SEXP E0, SEXP I0, SEXP R0,
                    SEXP nbCONTACT0, SEXP nbCONTACT1, SEXP nbMulCONTACT,
                    SEXP phiMIN, SEXP phiMAX, SEXP probVISITER, SEXP probINFECTER, SEXP tmax,SEXP nbVilles,
                    SEXP unitTIME, SEXP typeRNG, SEXP periDISE, SEXP statline)
{
    try{        
        //-sigma 0.125   //sigma
        ...
        //-S0 74150
        if (!isReal(S0)  ||  length(S0) != 1) {
            error("invalid value of S0,it should be a real number");
        }
        unsigned long cS0=NUMERIC_VALUE(S0);

        //-E0 270
        if (!isReal(E0)  ||  length(E0) != 1) {
            error("invalid value of E0,it should be a real number");
        }
        unsigned long cE0=NUMERIC_VALUE(E0);

        //-I0 160
        if (!isReal(I0)  ||  length(I0) != 1) {
            error("invalid value of I0,it should be a real number");
        }
        unsigned long cI0=NUMERIC_VALUE(I0);

        //-R0 925390
        if (!isReal(R0)  ||  length(R0) != 1) {
            error("invalid value of R0,it should be a real number");
        }
        unsigned long cR0=NUMERIC_VALUE(R0);
        ...
        //-phiMIN 0.0
        if (!isReal(phiMIN)  ||  length(phiMIN) != 1) {
            error("invalid value of phiMIN,it should be a real number");
        }
        double cphiMIN=NUMERIC_VALUE(phiMIN);

        //-phiMAX 0.0
        if (!isReal(phiMAX)  ||  length(phiMAX) != 1) {
            error("invalid value of phiMIN,it should be a real number");
        }
        double cphiMAX=NUMERIC_VALUE(phiMAX);
        ...
        double *rstatline = REAL(statline);
        double* arr_statDIS; create1D(cnbVilles,0.0,arr_statDIS);
        for(int i=0; i<cnbVilles; i++) arr_statDIS[i] = rstatline[i];
    /*
        cout<<"DISTRIBUTION stationaire"<<endl;
       ...
       // double **matrDIS;    create2D(cnbVilles,cnbVilles,0.0,matrDIS);
            //creating the matrix foloowing a uniforme distribution
        //matrDIS = createUnifNumb(cnbVilles,(double)1/cnbVilles,0,1);
        /*
        cout<<"matrix de transition"<<endl;
        ...
        cout<<endl;
          cout<<"khkhkkhhkhk"<<endl;
          */

            // finding the stationary distribution
       // double ** dismatrix=exponentMatrix(matrDIS,cnbVilles,100);
       // double* arr_statDIS=stationarydist(dismatrix,cnbVilles);
        ...
        for (int i = 0; i <  cnbVilles; i++)
        {
            ...
            for(int j = 0; j< nvar ; j++) valSEIR0[i][j] = 0;
        }
            // from the stationary matrix
        initializeStatSEIRYANN(cnbVilles,arr_statDIS,cS0,cE0,cI0,cR0,valSEIR0);
       ...
       // double nbCONTACT0 = valParSIM[inbCONTACT0];
        ...
        //double nbMulCONTACT = valParSIM[inbMulCONTACT];
        ...
        */
        ...
        // Doing simulation by using the function 'simulerSEIR'
         resMETAPOP resTOTAL =  simulerSEIRYANN(cnbVilles,csigma,cgamma,cmu,valSEIR0,
                                                                   0.0,0.0,cphi,cseed,cunitTIME,ctmax,ctypeRNG,
                                                                   cPeriDISE,arr_probVISITER,arrNbCONTACT0,arrNbCONTACT1,arr_probINFECTER,arr_probVENIRk);
