...
#
is.integer0NewInfec <- function(x)
{
...
initVarSEIRNNewInfec <- function(nbVilles=1,S=NULL,E=NULL,I=NULL,R=NULL,N=NULL,
	...
	if(nbVar==0) stop("There are not any initial variables!")
		...
		if(is.integer0NewInfec(which(vecVar==S))){ # missing  S
			#print("missing  S")
			S<-N-E-I-R	
			return(list(S=S,E=E,I=I,R=R,N=N))	
		}
		else if(is.integer0NewInfec(which(vecVar==E))){# missing  E
			#print("missing  E")
			E<-N-S-I-R
			return(list(S=S,E=E,I=I,R=R,N=N))	
		}
		else if(is.integer0NewInfec(which(vecVar==I))){# missing  I
			#print("missing  I")
			I<-N-S-E-R
			return(list(S=S,E=E,I=I,R=R,N=N))	
		}
		else if(is.integer0NewInfec(which(vecVar==R))){# missing  R
			#print("missing  R")
			R<-N-S-I-E	
			return(list(S=S,E=E,I=I,R=R,N=N))	
		}
		else if(is.integer0NewInfec(which(vecVar==N))){# missing  N
			#print("missing  N")
			N<-S+E+I+R
			return(list(S=S,E=E,I=I,R=R,N=N))	
		}
...
# 
globSEIRNewInfec<-function(type="stoch",duration=5*365,method="direct",unitTIME=1,mu=1/(70*365),
			nbCONTACT0=1000/365,nbCONTACT1=0.0,probINFECTER=0.01,probVISITER=0.01,			
			sigma=1/8,gamma=1/5,
			periDISE=365,phiPHASE=c(0),nbVilles=1,seed=as.numeric(Sys.time()),typeRNG="good",
			S=NULL,E=NULL,I=NULL,R=NULL,N=1e5,...){
	# stochastic simulation
	if(!is.integer0NewInfec(grep(type,"stochastic")))
	...
	else if(!is.integer0NewInfec(grep(type,"deterministic")))
...
stoSEIRNewInfec<-function(sigma=1/8,gamma=1/5,mu=1/(70*365),seed=as.numeric(Sys.time()),S=NULL,E=NULL,I=NULL,R=NULL,N=1e5,
	...
	if(!is.integer0NewInfec(grep(method,"direct"))){
		if(!is.integer0NewInfec(grep(typeRNG,"good"))) rtypeRNG=0
		else
		  if(!is.integer0NewInfec(grep(typeRNG,"fast"))) rtypeRNG=1
		else
	...
	if(!is.integer0NewInfec(grep(method,"adaptivetau"))){	
		flagSEIR = TRUE;
		for(i in 1:nbVilles){
		...
		if(flagSEIR){# stochastic simulation for SEIR model
			...
			allPops <- .Call('adapNewInfec',init.values,NULL,NULL, 
			      #params,
	...
	return(new("seirNewInfec",sigma=sigma,gamma=gamma,mu=mu,seed=seed,S=initS,E=initE,I=initI,R=initR,N=initN,
...
function(object,x="time",y="P",type="s",col="red",xlim=c(),ylim=c(),curvetype="KM",vilabline=c(),add=F,
	...
	if(leng>0){# if there is extinction
		...
		if(!is.integer0NewInfec(grep(curvetype,"KM"))){
		...
		if(!is.integer0NewInfec(grep(curvetype,"population"))){
			...
			if(is.null(pop)){# 'pop', population that we want plot
				...
				for(i in 1:nbvilabline){# finding position of extinction
					flagEX_i <- FALSE
					if(!is.integer0NewInfec(which(pop==vilabline[i]))){
						for(j in 1:nbVilExt)
							if(vilabline[i]==allVillesEX[j]){
								abline(v=pers$time[j])	
								flagEX_i=TRUE
