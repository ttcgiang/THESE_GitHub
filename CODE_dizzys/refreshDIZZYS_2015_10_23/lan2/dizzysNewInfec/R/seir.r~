...
#
initVarSEIRN <- function(nbVilles=1,S=NULL,E=NULL,I=NULL,R=NULL,N=NULL,
	...
	phiPHASE<-resizeVector(V=phiPHASE,n=nbVilles)
	...
	if(nbVar==0) stop("There are not any initial variables!")
		...
		# then get initial valeus for variables at the last time 
			S<-c(); E<-c(); I<-c(); R<-c()
			for(i in 1:nbVilles){				
				valEqual <- equilibrium(duration=100*365,N=N[i],mu=mu[i],nbCONTACT0=nbCONTACT0[i],probINFECTER=probINFECTER,sigma=sigma[i],gamma=gamma[i],phiPHASE=phiPHASE[i],periDISE=periDISE)
...
# 
seirGeneral<-function(type="stoch",duration=5*365,method="direct",unitTIME=1,mu=1/(70*365),nbCONTACT0=1000/365,nbCONTACT1=0.0,probINFECTER=0.01,sigma=1/8,gamma=1/5,
	...
	if(!is.integer0(grep(type,"stochastic")))
		seirStoch(method=method,duration=duration,unitTIME=unitTIME,mu=mu,nbCONTACT0=nbCONTACT0,nbCONTACT1=nbCONTACT1, probINFECTER=probINFECTER,
	...
	# deterministic simulation
		...
		seirDet(nbVilles=nbVilles,duration=duration,unitTIME=unitTIME,S=S,E=E,I=I,R=R,N=N,periDISE=periDISE,
...
seirDet<-function(nbVilles=1,duration=10*365,unitTIME=1,S=NULL,E=NULL,I=NULL,R=NULL,N=1e7,periDISE=365,
	...
	# phases	
	phiPHASE<-resizeVector(phiPHASE,nbVilles); #phiPHASE
	# initial values for all variables
	initVar<-initVarSEIRN(nbVilles=nbVilles,S=S,E=E,I=I,R=R,N=N,mu=mu,nbCONTACT0=nbCONTACT0,nbCONTACT1=nbCONTACT1,probINFECTER=probINFECTER,
...
seirStoch<-function(sigma=1/8,gamma=1/5,mu=1/(70*365),seed=as.numeric(Sys.time()),S=NULL,E=NULL,I=NULL,R=NULL,N=1e5,
		nbCONTACT0=300, nbCONTACT1=0.1, nbMulCONTACT=1, phiPHASE=c(0),
		probVISITER=0.01, probINFECTER=0.01, duration=1000, nbVilles=2, unitTIME=1,periDISE=365,
	...
	if(statSTATE){
		...
		initVar<-initVarSEIRN(nbVilles=1,S=S,E=E,I=I,R=R,N=N,mu=mu,nbCONTACT0=nbCONTACT0,nbCONTACT1=nbCONTACT1,sigma=sigma,gamma=gamma,periDISE=periDISE,phiPHASE=phiPHASE)
	...
	if(!is.integer0(grep(method,"direct"))){
		...
		resMetaPOP <- .Call("getValeurPOPSYANN",sigma,gamma,mu,seed,initS,initE,initI,initR,
				nbCONTACT0,nbCONTACT1,nbMulCONTACT,phiPHASE,probVISITER,probINFECTER,
	...
	return(new("seir",sigma=sigma,gamma=gamma,mu=mu,seed=seed,S=initS,E=initE,I=initI,R=initR,N=initN,
		nbCONTACT0=nbCONTACT0, nbCONTACT1=nbCONTACT1, nbMulCONTACT=nbMulCONTACT, phiPHASE=phiPHASE,
		probVISITER=probVISITER, probINFECTER=probINFECTER, duration=duration, nbVilles=nbVilles, unitTIME=unitTIME,periDISE=periDISE,		typeRNG=typeRNG,typeSIMU="stochastic",method="direct",statSTATE=statSTATE,localExtPOP=listLocalEXTPOP,disRecolPOP=listDisRecolPOP,
		pop=populations))
#the end
}
