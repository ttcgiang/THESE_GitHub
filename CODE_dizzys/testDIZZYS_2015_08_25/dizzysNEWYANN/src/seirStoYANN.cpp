...
void calLamdaYANNGIANG(int nbVilles, double** prbVisit,unsigned long** mtVarb,
...
*/
  ...
  */
   ...
   */
    ...
    void initializeStatSEIRYANN(int nbVilles, double* statMatrix, unsigned long S0, unsigned long E0,unsigned long I0, unsigned long R0,
                            unsigned long **&y){
        for(int i=0; i<nbVilles; i++){
   ...
   */
           ...
           if(typeRNG==FAST) {// using the method of Yann
               ...
               if(typeRNG==GOOD){// using the method of C++
                   ...
                   //SENARION
                  // p1 = 0.01 + (rand()/(double)RAND_MAX) * (0.1 - 0.01);
                  // p2 = 0.001 + (rand()/(double)RAND_MAX) * (0.01 - 0.001);
                   //p3 =0.5 + (rand()/(double)RAND_MAX) * (1.0 - 0.5);
               //    cout<<"p1="<<p1<<endl;
                // cout<<"p2="<<p2<<endl;
                 //cout<<"p3="<<p3<<endl;
