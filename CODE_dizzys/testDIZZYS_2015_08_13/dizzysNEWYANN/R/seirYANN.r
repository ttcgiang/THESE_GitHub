...
seir.stochYANN<-function(sigma=1/8,gamma=1/5,mu=1/(70*365),seed=as.numeric(Sys.time()),S=NULL,E=NULL,I=NULL,R=NULL,N=1e5,
	...
	lsLocalEXTDIS <- vector("list",nbVilles)
	lsRecolEXTDIS <- vector("list",nbVilles)
	for( ivil in 1:nbVilles){
		nbRecolEXT<-length(resRecol[[ivil]])
		nbLocalEXT<-length(resLocalExt[[ivil]])
		if(nbRecolEXT==0){
			lsLocalEXTPOINT[[ivil]]<-NULL
			lsRecolEXTPOINT[[ivil]]<-NULL
			lsLocalEXTDIS[[ivil]]<-NULL
			lsRecolEXTDIS[[ivil]]<-NULL
		}
		else{
			newLocalExtPOPSTART<-resLocalExt[[ivil]][1:nbRecolEXT]
			newLocalExtPOPEND<-resLocalExt[[ivil]][2:nbLocalEXT]
			#
			lsLocalEXTPOINT[[ivil]]<-resLocalExt[[ivil]]
			lsRecolEXTPOINT[[ivil]]<-resRecol[[ivil]]
			#
			vecLocalEXT<-unlist(newLocalExtPOPEND-resRecol[[ivil]])
			lsLocalEXTDIS[[ivil]]<-subset(vecLocalEXT,vecLocalEXT>0)
			#
			vecRecolEXT<-unlist(resRecol[[ivil]]-newLocalExtPOPSTART)
			lsRecolEXTDIS[[ivil]]<-subset(vecRecolEXT,vecRecolEXT>0)
	...
	return(new("seirYANN",sigma=sigma,gamma=gamma,mu=mu,seed=seed,S=S0,E=E0,I=I0,R=R0,N=N0,
		...
		lsLocalEXTDIS=lsLocalEXTDIS,lsRecolEXTDIS=lsRecolEXTDIS,pop=populations))
#the end
}
